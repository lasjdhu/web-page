...
export function useTerminal({
  ...
  const ls = (path: string, args: string[]): string => {
    ...
    const showHidden =
      args.includes("-a") || args.includes("-al") || args.includes("-la");
    const showDetails =
      args.includes("-l") || args.includes("-al") || args.includes("-la");
  ...
  time`,
      ...
      output = new Intl.DateTimeFormat("en-US", dateFormatOptions).format(
        new Date(),
      );
    }
  ...
  const handleCommand = (cmd: string): { output: string; type: EntryType } => {
    ...
    switch (baseCommand) {
      ...
      case "pwd":
        if (args.length > 0)
          return { output: "pwd: expected 0 arguments; got 1", type: "error" };
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>): void => {
    ...
    if (e.key === "Enter" && !e.shiftKey) {
      e.preventDefault();
      if (input.trim()) {
        const existingIndex = commandHistory.findIndex(
          (entry) => entry === input,
        );
  ...
  const handleInputChange = (
    ...
    if (!newValue.startsWith(promptPrefix)) {
      ...
      setCaretPosition(
        Math.max(0, e.target.selectionStart - promptPrefix.length),
      );
    }
